...
(async()=>{
 const db=new PGlite();
 try{
  await db.exec("create role anon; create role authenticated; create schema auth; create table auth.users(id uuid primary key); create function auth.uid() returns uuid language sql stable as $$ select nullif(current_setting('request.jwt.claim.sub',true),'')::uuid $$; revoke usage on schema public from public,anon,authenticated; grant usage on schema auth to anon,authenticated; grant execute on function auth.uid() to anon,authenticated;");
  ...
  await db.exec('revoke select on public.damda_shared_cards from authenticated');
  await assert.rejects(()=>as('authenticated',first,tx=>tx.query('insert into public.damda_shared_cards(id,owner_id,data) values ($1,$2,$3) returning id',[cardId,first,{name:'Public A',fields:[]}])),/permission denied/);
  const repair=await fs.readFile('supabase/migrations/202609200001_fix_shared_cards_grants.sql','utf8');await db.exec(repair);await db.exec(repair);
  assert.deepEqual((await as('authenticated',first,tx=>tx.query('insert into public.damda_shared_cards(id,owner_id,data) values ($1,$2,$3) returning id',[cardId,first,{name:'Public A',fields:[]}]))).rows,[{id:cardId}]);
  ...
  assert.equal((await as('anon','',tx=>tx.query('select public.damda_get_shared_card($1) as card',[cardId]))).rows[0].card,null);
  console.log('PASS (local PostgreSQL): all migrations, idempotent grants repair, INSERT RETURNING, own-account read/write, cross-account denial, anonymous table denial, exact-ID public card RPC, unknown link, owner-only revoke, spoofed owner denial.');
