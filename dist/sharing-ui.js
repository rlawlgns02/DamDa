...
function showShare(){
  ...
  const native=document.getElementById('native-share');if(native)native.onclick=async()=>{try{await navigator.share({title:'FOLD 명함',url});}catch(error){if(error.name!=='AbortError')toast('링크 복사를 이용해 주세요.');}};
}
...
async function downloadReceived(format,button){
  ...
  try{const blob=format==='png'?await FoldExport.png(data):new Blob([FoldExport.vcard(data)],{type:'text/vcard;charset=utf-8'});FoldExport.download(blob,FoldExport.filename(data,format));toast('다운로드를 시작했어요.');}
