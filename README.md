...
자동 저장에 실패하면 재시도할 수 있습니다. 다른 기기의 편집과 충돌하면 현재 내용을 백업한 뒤 서버 데이터를 다시 불러올 수 있습니다. 저장에 실패한 상태에서도 백업하거나 미저장 변경을 버리고 로그아웃할 수 있습니다.

로그인 없이 체험한 내용은 현재 브라우저에 저장됩니다. 공유 링크·QR은 가입 후 생성할 수 있으며, 체험 화면에서 가입하면 작성한 프로필과 명함함을 새 계정에 저장합니다.
...
`server.cjs`는 별도의 과거 로컬 체험 서버이며 운영 Auth를 제공하지 않습니다. 운영 화면은 `npm start` 또는 GitHub Pages를 사용합니다.
...
브라우저 테스트는 기본적으로 설치된 Microsoft Edge를 사용합니다. 다른 Playwright 브라우저 채널은 DAMDA_BROWSER_CHANNEL로 지정합니다. 스크린샷 저장 위치는 DAMDA_TEST_ARTIFACT_DIR로 지정할 수 있습니다.

`npm test`는 기존 계정 데이터 변환과 공개 범위 보존, 계정 전환 중 공유·저장 격리, 부분 저장과 로그아웃 복구를 검증합니다. test:cloud(test:browser와 동일)는 실제 Supabase SDK와 모의 API로 가입·로그인·복구·자동 저장·충돌·계정 분리·공유·다운로드 및 반응형 화면을 검증합니다. 브라우저 테스트는 종료 시 기존 연결 설정으로 자동 재빌드하며, 실행별 스크린샷은 `.tmp-browser/production-*` 폴더에 저장합니다. test:db는 로컬 PostgreSQL 엔진에서 전체 migration, 권한 복구 반복 적용, INSERT RETURNING과 RLS를 검증합니다. 운영 계정 인증과 이메일 발송은 배포 대상 Supabase에서도 별도로 확인해야 합니다.
